...
def is_valid_next_section(curr_key, next_key):
    """
...
#  pdf_path = sys.argv[1]
pdf_path = os.environ.get("SPEC_WIFI")
...
if match:
  ...
  #print("extracted page number", extracted_page_number)
...
#      extracted_page_number = int(match.group())
    ...
    for line in cleaned_lines:
        ...
        new_heading = 0
        #print("line", line)
        if clause_pattern.match(line):  # Found a new section or sub-section
          parts = line.split()
          ...
          if is_valid_next_section(curr_key, next_key):
            new_heading = 1;
        ...
        if new_heading == 1 and clause_pattern.match(line):  # Found a new section or sub-section
          if current_section is not None:
              ...
              print(f"✅ Finalized Section: {current_section_number} ({current_title}) - Pages {current_page_start} to {page_num}")
...
#          current_title = line  # Full section title
          ...
          print(f"📌 New Section Detected: {current_section_number} ({current_title}) - Starting at Page {current_page_start}")
...
if current_section:
    ...
    print(f"✅ Finalized Last Section: {current_section_number} ({current_title}) - Pages {current_page_start} to {len(doc)}")
...
print(f"🔹 Sample chunk: {initial_chunks[0]}")
