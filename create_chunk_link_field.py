...
def find_fields_in_content(input_json_file, fields_json_file, output_csv_file):
    ...
    field_dict = defaultdict(int)  # auto-initialize counts to 0
...
# Convert all section_titles to lowercase for the frequency count
       low_freq_titles = {key.lower() for key, freq in field_dict.items() if freq < 5}
...
# Save back to JSON
       ...
       print(f"✅ Filtered JSON written to {fields_json_file}")

input_chunk_file = os.environ.get("SPEC_CHUNKS_TRIM")
input_field_list = os.environ.get("FIELDS_LIST")
chunk_link_field = os.environ.get("CHUNK_LINK_FIELD")

# In first iteration, we remove the field that does not have links with chunks. reference count < 2 
print("Ist Iteration...")

find_fields_in_content(input_chunk_file, input_field_list, chunk_link_field)


# In second iteration, we reform the chunks-to-field link using the new cleaned field list
