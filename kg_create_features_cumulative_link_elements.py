...
chunks_to_frames_file = os.getenv("CHUNK_LINK_ELEMENT")
features_file = os.getenv("KG_TOP_FEATURES")
output_file = os.getenv("KG_FEATURE_ELEMENT_SCORE")
...
for _, feature in features_df.iterrows():
   ...
   # matched_frames = frames_df[frames_df["section_number"].str.startswith(feature_sec)]
    ...
    for name, group in matched_frames.groupby("name"):
...
elements_chunk_counts = frames_df.groupby("name")["chunkid"].nunique()

# Compute global scores
global_scores = (total_chunks / elements_chunk_counts).to_dict()

# Add global score to output_df
output_df["global_score"] = output_df["name"].map(global_scores)
